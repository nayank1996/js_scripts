// ==UserScript==
// @name         [Optimus] Auto Compare Metadata across EN locale
 ...
 var $ = window.jQuery;
    setTimeout(function(){
        ...
        function processJSON_Response (rspObj) {
            ...
            checkDuplicateItem(descriptionPageInfo, "description");
            checkDuplicateItem(descriptionPageInfo, "examplePhrases");
            checkDuplicateImage(mediaInfo);

            if (skillType == "CUSTOM"){
                descriptionPageInfo = rspObj.response.skillManifest.skillManifestFeatures.apis.custom.locales
                checkDuplicateItem(descriptionPageInfo, "invocationName")
            }
        }

        function checkDuplicateItem(descriptionPageInfo, typeOfItem){
            ...
            outPutResultArray.forEach(result => {
                outPutText = outPutText + result.locale + " have same " + typeOfItem + "\n"
            })
