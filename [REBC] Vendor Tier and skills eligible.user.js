// ==UserScript==
// @name         [REBC]Vendor Tier/Skills eligible
...
(function() {
    ...
    let exclBadge = ['ISP','AMAZON_PAY', 'FINANCIAL_SKILL', 'NI','CCP','INCORRECT_ZTP_FUNCTIONAL','INCORRECT_ZTP_SECURITY','INCORRECT_ZTP_SLU','COLLECTS_USER_INFORMATION','VerifiedHighRisk_Security','PaidSkill','ChildDirected','CHILD_DIRECTED','VerifiedHighRisk_Policy', 'DRS_RelationshipModel_HighRisk_Security', 'DRS_RelationshipModel_HighRisk_Policy']
    ...
    .then(vendorId => {
        ...
        .then(result => {
           ...
           if(result.data.length > 0){
                  if(result.data[0].dev_tier_level !='Tier_3'){val++
                                                        setTimeout(function () {
   ...
   GM_xmlhttpRequest({
    ...
    fetch('https://alexaskillcontent-ca-iad.iad.proxy.amazon.com/api/cops-manifests/certification/'+skillId+'/'+skillVersion+'?withCopsDerivedData=true')
        .then(response => response.json())
        .then(function(data) {
        if(data.skillManifest.skillTypes.includes('SMART_HOME')){ val++
       setTimeout(function () {
      ...
      newData0.html('    ||Not A REBC Skill These Badges found SMART_HOME')
...
console.log(badges)
    ...
    .then(locales => {
           ...
           let locales11 = ['en-CA', 'en-AU', 'en-US', 'en-GB', 'en-IN', 'de-DE', 'ja-JP']
           let arr1 = []
    ...
    .then(data => {
       ...
       if(stat == "ARCHIVED" || stat == "LIVE" || stat == "SUPPRESSED"){
        ...
        let newData = $("<span>").css("background-color", "rgb(238, 130, 238)")
         newData.html('   |||version upgrade Not A REBC Skill')
